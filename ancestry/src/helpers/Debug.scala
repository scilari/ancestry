...
object Debug {
  def debugString[T](
    ...
    def rec(tree: Tree[T], intendation: Int): String = {
      val prefix =
        if (intendation > 0) " " * (intendation - INTENT_PER_LEVEL) + "\\" + "." * (INTENT_PER_LEVEL - 1) else ""
      val tag = prefix + toString(tree) + s" (${tree.parent.map { p => toString(p) }.getOrElse("root")})" + "\n"
      tree match {
        case _: Leaf[T]   => tag
        case b: Branch[T] => tag + b.children.map(c => rec(c, intendation + INTENT_PER_LEVEL)).fold("")(_ + _)
      }
    }
